      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h2 className="font-display text-4xl sm:text-7xl font-black text-white mt-4 tracking-tight">
            CLAIM YOUR <span className="text-yellow-400 text-glow-white">RANK</span>
          </h2>
          ...
          <div className="inline-flex items-center space-x-2 my-6 px-6 py-2.5 rounded-2xl bg-gradient-to-r from-amber-600 via-yellow-500 to-amber-600 text-black font-display font-black text-xl sm:text-2xl shadow-[0_0_30px_rgba(234,179,8,0.6)]">
            <Sparkles className="w-6 h-6 fill-current animate-spin" />
            <span>{eventConfig.prizes.totalCashPool}</span>
          </div>
        </div>

        {/* 3D Rising Podium Composition (Center Champion Tallest) */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-end max-w-5xl mx-auto my-16">
          
          {/* #02 RUNNER-UP (Left Column) */}
          <motion.div
          ...
          <motion.div
            ...
            onMouseEnter={() => soundEngine.playClick()}
            className="order-1 md:order-2 p-8 rounded-3xl bg-gradient-to-b from-[#1c1827] via-[#121118] to-[#1c1827] border-4 border-yellow-400 shadow-[0_0_60px_rgba(234,179,8,0.6)] flex flex-col justify-between text-left h-[30rem] scale-105 z-20 relative interactive-card"
          >
          ...
          {/* #03 SECOND RUNNER-UP (Right Column) */}
          <motion.div
            initial={{ opacity: 0, y: 60 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
        ...
        {/* Special Category Awards — Japanese Hanko Stamp Emblem Badges */}
        <div className="mt-20">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
            {eventConfig.prizes.specialTitles.map((sp) => {
              ...
              return (
                ...
                >
                  {/* Japanese Hanko Ink Stamp Watermark */}
